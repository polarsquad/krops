...
| ACK controllers on the management cluster | ASO 2.19.0 Helm release on workload clusters |
| Static SOPS credential (no per-cluster identity) | Workload identity: user-assigned identity + federated credential + role assignment, reconciled by the ASO bundled with CAPZ on mgmt |

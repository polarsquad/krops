...
# The Bucket CRs live on the management cluster, which is deleted before this
# sweep runs, so the buckets are orphaned. Buckets are versioned
# (see mgmt/aws/infrastructure/workload-resources/bucket.yaml), so every
# object version AND delete marker must be purged before the bucket itself can
# be deleted.
_cleanup_s3_bucket() {
